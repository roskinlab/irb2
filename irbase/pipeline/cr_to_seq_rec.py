...
def main():
    ...
    elif args.both_chains:
        chains_to_parse = {'IGH', 'IGK', 'IGL'}
    else:   # default, same as above
        chains_to_parse = {'IGH', 'IGK', 'IGL'}

    logging.info('conveting these chain types: %s', chains_to_parse)
    logging.info('reading annotations from: %s_contig_annotations.csv', args.group)
    logging.info('reading sequences from: %s_contig.fastq', args.group)
    ...
    seq_records = make_seq_records(cell_umi_count, cell_contig, sequences, annotations,
                                   subject=args.subject, sample=args.sample, source=args.source,
                                   drop_contig=args.drop_contig)

    writer(sys.stdout.buffer, output_schema, seq_records, codec=args.codec)

    logging.info('converted %d sequences', len(cell_umi_count))
